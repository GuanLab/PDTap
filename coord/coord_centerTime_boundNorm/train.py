...
# Generate data
def data_generator(paths, bounds, labels, batch_size, is_train):
    
    assert paths.shape[0] == bounds.shape[0] == labels.shape[0]

    for index in range(0, paths.shape[0], batch_size):

        if index + batch_size <= paths.shape[0]:
            batch_paths = paths[index:index+batch_size]
            batch_bounds = bounds[index:index+batch_size, :]
            batch_labels = labels[index:index+batch_size]
        else:
            return

        batch_vals = []
        for path, bound in zip(batch_paths, batch_bounds):
            
            # some errors here
            try: 
                coord_data = np.load(path)
            except:
                print(path)
                sys.exit()
            #######
            ...
            # bound normalization
            bound_normalization(coord_data[:, 1:], bound)
            # centering time
            coord_data[:, 0] -= np.mean(coord_data[:, 0], axis=0)

            coord_data = fix_size(coord_data[:, [0, 2, 3]].T, size=size)

            if is_train:
                # time scaling
                scale = random.random() * (max_scale - min_scale) + min_scale
                coord_data = scale_time(coord_data, scale)
                ...
                b = random.random()
                c = random.random()
                coord_data = quarternion_rotation(coord_data, theta, a, b, c)

                # scale_magnitude
                scales = [random.random() * (max_scale - min_scale) + min_scale for _ in range(3)]
                scale_magnitude(coord_data, scales)

            batch_vals.append(coord_data)
...
def train(input_file, model_num, model_name):
    ...
    path_train, bound_train, path_validate, bound_validate, label_train, label_validate = train_validate_split(input_file, model_num, 0.5)
    ...
    learning_rate = theano.shared(lasagne.utils.floatX(5e-5))
    ...
    LOG = open('%s_%s_train.log' % (step, model_name), "w") 
    ...
    while cur_epoch <= epoch_num:
        ...
        for (train_vals, train_labels), (test_vals, test_labels) \
                in tqdm(izip(data_generator(path_train[array_order], bound_train[array_order], label_train[array_order], batch_size, False), \
                data_generator(path_validate, bound_validate, label_validate, batch_size, False))):
