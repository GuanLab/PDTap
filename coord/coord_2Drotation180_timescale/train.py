...
model_file = args['model']  # guan_version1_800
...
def data_generator(paths, demos, labels, batch_size, is_train):
    ...
    for index in range(0, paths.shape[0], batch_size):
        ...
        for path in batch_paths:
            ...
            # No normalization
            # coord_data = normalization(coord_data)
            coord_data = fix_size(coord_data.T, size=size)
            ...
            if is_train:
                ...
                # 2d rotation
                theta = random.uniform(-1.0, 1.0) * math.pi * 0.5
                coord_data = rotation_2d(coord_data, theta)
...
def train(input_file, model_num, model_name):
    ...
    while cur_epoch <= epoch_num:
        ...
        for (train_vals, train_demos, train_labels), (test_vals, test_demos, test_labels) \
                in tqdm(izip(data_generator(path_train[array_order], demo_train[array_order], label_train[array_order], batch_size, True), \
