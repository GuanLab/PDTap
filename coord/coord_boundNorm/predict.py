...
if __name__ == '__main__':
    ...
    with open(test, "r") as TEST:
        for line in tqdm(TEST):
            ...
            accel_pred = np.load(table[2])
            # bound normalization
            bounds = table[3:5]
            bound_normalization(accel_pred[:, 1:], bounds)
            accel_pred = fix_size(accel_pred[:, 2:].T, size)

            accel_pred = accel_pred[:, :, np.newaxis].transpose((2, 0, 1)).astype("float32")

            predictions = pred(accel_pred)
            eva.write("\t%.4f\n" % predictions)
