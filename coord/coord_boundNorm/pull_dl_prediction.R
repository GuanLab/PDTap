...
colnames(test_data) = c("recordId", "label", "file", "TappedButtonLeft", "TappedButtonRight",
                        "age", "male", "female", "medical.true", "medical.false",
                        "eva1", "eva2", "eva3", "eva4", "eva5")

avg = mean(colMeans(test_data[, c("eva1", "eva2", "eva3", "eva4", "eva5")], na.rm=TRUE))

## pull_dl_test
feature_test = data.frame(label=test_data$label,
                          eva=apply(test_data[, c("eva1", "eva2", "eva3", "eva4", "eva5")], 1, mean, na.rm=TRUE))
