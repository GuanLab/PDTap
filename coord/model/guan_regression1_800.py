...
def network(input_var, label_var, shape):
    ...
    loss = nn.objectives.squared_error(output, label_var).mean() #, ss=ss)
    te_loss = nn.objectives.squared_error(output_det, label_var).mean() #,ss=ss)
    # te_acc = nn.objectives.binary_accuracy(output_det, label_var).mean()

    return layer, loss, te_loss
