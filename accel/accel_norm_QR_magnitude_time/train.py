...
def run_epoch(input_var,label_var,fn,if_train):
    ...
    while (i<count):
        ...
        while (jjj<(i*batch+batch)):
                ...
                if (if_train==1):
                    seq=scaleImage(seq,rrr_scale) #this line was resizing the length
                    ...
                    rrr=random.random()
                    rrr_scale=rrr*(max_scale-min_scale)+min_scale
                    seq[0,:]=seq[0,:]*rrr_scale
                    rrr=random.random()
                    rrr_scale=rrr*(max_scale-min_scale)+min_scale
                    seq[1,:]=seq[1,:]*rrr_scale
                    rrr=random.random()
                    rrr_scale=rrr*(max_scale-min_scale)+min_scale
                    seq[2,:]=seq[2,:]*rrr_scale
...
def run_params(train_input_var, train_label_var, test_input_var, test_label_var,fold):
    ...
    lr = theano.shared(lasagne.utils.floatX(5e-5))
    ...
    while (epoch<=max_epoch):
        ...
        a=range(len(train_input_var))
        np.random.shuffle(a)
        i=0
        new_train_input_var=[]
        new_train_label_var=[]
        while (i<len(a)):
            new_train_input_var.append(train_input_var[a[i]])
            new_train_label_var.append(train_label_var[a[i]])
            i=i+1
## train_input_var is a name, so that sub-sequence can be selected there;
        new_train_input_var=np.asarray(new_train_input_var)
        new_train_label_var=np.asarray(new_train_label_var)

        tr_err = run_epoch(new_train_input_var,new_train_label_var, train_fn,1)
...
(train_input_var, train_label_var, test_input_var, test_label_var)=GS_split.GS_split(sys.argv[1],500,1)
run_params(train_input_var, train_label_var, test_input_var, test_label_var,'fold1')
(train_input_var, train_label_var, test_input_var, test_label_var)=GS_split.GS_split(sys.argv[1],500,2)

run_params(train_input_var, train_label_var, test_input_var, test_label_var,'fold2')
(train_input_var, train_label_var, test_input_var, test_label_var)=GS_split.GS_split(sys.argv[1],500,3)

run_params(train_input_var, train_label_var, test_input_var, test_label_var,'fold3')
(train_input_var, train_label_var, test_input_var, test_label_var)=GS_split.GS_split(sys.argv[1],500,4)

run_params(train_input_var, train_label_var, test_input_var, test_label_var,'fold4')
(train_input_var, train_label_var, test_input_var, test_label_var)=GS_split.GS_split(sys.argv[1],500,5)
